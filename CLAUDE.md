...
- `UISelectable` — `Select(bool)` (patch target; the unified selection chokepoint reached by both navigation and programmatic/initial focus, which bypasses `UICamera.SetSelection`), `IsValid`, `IsSelectable`. `UISelection.SelectedObject`/`UISelectionManager` are the layered selection model behind it (Primary/Secondary/Dialogue categories). Also patched: `OnKey` (bool prefix; consumes the map cursor's Ctrl+arrows, and while the sound glossary overlay is open records arrows/Escape into `GlossaryState` and consumes them - UICamera funnels every device into the selection's OnKey, so this one capture covers keyboard, WASD, dpad, stick, B and Start) and `DoClick` (bool prefix; while the glossary is open records the confirm and consumes it, otherwise marks the frame user-driven for the focus announce policy). The glossary's pause-menu option is injected by `GlossaryButton` through public game API only (`PauseMenuManager.CurrentState.Controller`, `PauseMenuButtonSet.Buttons`, `UISelectableGroup.Add`, the public `selectOnUp/Down` links and `ClickAction`/`CancelAction` delegates on `UISelectable`) - no reflection, but audit those members after a game update. `UISelection.m_selectionBlockerGroup` is read in `ProxyFactory` to mirror the game's blocked-selection check.
...
- `RevealMapLayoutSlots.Show(List<MapLayoutSlot>)` (patch target; the single coroutine every map reveal effect funnels through, and the only place the game sets `EncounterCard.Revealed = true`) — an iterator, so the postfix fires at creation; it records the live slot list in `MapReveal` and the pump announces once every card's public `Revealed` flag is set, since the game locks selection to a bare confirm while the cards flip.
...
- Zoom/examine readers — `CardContainer.m_selectableContextTitle` / `m_selectableContextTitleParams` / `m_selectableContextText` / `m_cancelContextText` / `m_cardClickAction` / `m_cardCancelAction` (`ZoomReader`, the zoomed card's action prompts); the hint's confirm/cancel key names come from the public `UIInputSprite.ControllerButtonString` / `KMButtonString` on `UIManager.Instance.PrimaryNavBar`'s `ConfirmButton` / `CancelButton`, gated on the public `InputManager.UseGamepad` (no reflection, but audit after a game update); `InfoPanel.m_title` / `m_statTitle` / `m_statValue` / `m_description` and `ComparePanelManager.m_old` (the equipment compare panel, also `ZoomReader`); `CabinetCardInfo.m_panelIndex` / `m_panels` (`CardInfoPanelReader`); `StartOptionsContainer.m_courtCardInfo` (`CabinetReader`).
...
- Traps (`ZoneSonification`'s second source) — `Trap.m_trapPhases` (read once per scan to spot `TrapPhaseWaitForTrigger`, the primed-idle traps) and `CombatApplicantTrigger.m_applyToTargets` (each trigger's armed/safe switch, polled per frame; arming retroactively hits anyone already inside). Traps have no registry, so they are found by `FindObjectsOfType<Trap>`, rescanned when `CombatEncounter.Instance` changes or when the `.combat`-group postfix on `Trap.Start` (an iterator, so it fires at activation) flags a new trap - chests/exits can enable trap hierarchies mid-level. Damage triggers sit on the trap's own object (confirmed in `ActionMoveAI`), reached via `GetComponentsInChildren` - plural, one voice per trigger.
- Beams and segment chains (`ZoneSonification`'s third source) — neither has a registry, so `.combat`-group postfixes record them: `CombatProxyBeam.Engage(ICombatProxyBeamParent, Vector3, float, float)` (a beam is NOT a CombatProxy; its team is read off the parent argument cast to `CombatProxy`) and the `OnEngage` overrides of `CombatProxyTrail` / `CombatProxyLightning` (both verified to declare one). Read per frame: `CombatProxyBeam.m_target` / `m_time` / `m_growTime` / `m_expireTime` (the authored local line endpoint and lifecycle; `m_expireTime` is a nullable, boxed null while live) and `CombatProxyTrail.m_segments` / `CombatProxyLightning.m_segments` (each chain's live segment list, one voice per chain at the nearest segment).
