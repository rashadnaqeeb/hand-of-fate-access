...
- Traps (`ZoneSonification`'s second source; no patch) — `Trap.m_trapPhases` (read once per scan to spot `TrapPhaseWaitForTrigger`, the primed-idle traps) and `CombatApplicantTrigger.m_applyToTargets` (the trap's armed/safe switch, polled per frame; arming retroactively hits anyone already inside). Traps have no registry, so they are found by `FindObjectsOfType<Trap>` rescanned when `CombatEncounter.Instance` changes; the damage trigger sits on the trap's own object (confirmed in `ActionMoveAI`), reached via `GetComponentInChildren`.
