...
- [ ] Map-explorer navigation keys (the Phase 5 consumer): right stick on controller, Ctrl+arrows on keyboard. Confirmed the game leaves the right stick free on the map (its only right-stick uses are shop item-rotate, deck-builder archetype-panel scroll, and credits scroll, none of which overlap the map), so scope the capture to the map screen via the `ScreenStack`. Plain arrows drive the game's own UI nav regardless of Ctrl, so the Ctrl+arrow fallback must consume the arrow (via the existing `UISelectable.OnKey` patch) on the map.
- **Proof:** a bound mod key fires its action *(status key confirmed in-game on `/` and right-stick click)*; the unclaimed-key fall-through is proven with the map cursor in Phase 5.
...
- [ ] Capture the map-cursor input scoped to the map screen: right stick on controller (free here, see Phase 4), Ctrl+arrows on keyboard (consuming the arrow via the `UISelectable.OnKey` patch so the game's nav doesn't also move).
- [ ] Free-roam grid cursor over `Map.Instance.MapLayout` via `GetSlot(Vector2)`, opened anchored on the player's slot (`PlayerCounter.CurrentSlot`). Empty grid cells (`AllowedCategories == 0`, destroyed at fill) read as gaps so the board's shape is perceivable.
- [ ] Per-slot readout reuses `MapSlotReader`, already corrected for the board: a card's identity is read only when face-up, where face-up = `Flipped XOR (Complete || Revealed)` (the layout's flip rule, confirmed against the live board: default cards are face-down, only completed/revealed/flipped ones show their face); otherwise it is withheld as face-down. Add the slot state the card alone does not carry: current (player is here) / completed / reachable (`IsUnlocked` and adjacent to the player) / locked, plus direction and distance to the player.
- [ ] Quick affordances for what a sighted player gets at a glance, so the player is not forced to sweep the whole grid for 2-3 options: a key for the reachable frontier (the unlocked adjacent moves) and the game's own suggested next step (`Map.UpdateInitialSelection` / `GetNextSlot`).
...
- **Proof:** a blind player can survey the whole visible board (layout, adjacency, position, the reachable frontier, and each face-up card) and move through it, without sight.
