...
namespace HandOfFateAccess.Screens {
	/// <summary>
	/// Reads the end-of-run scoreboard: the header line and each score row (title, score,
	/// optional sub-score). The rows are display-only; only the continue button is
	/// focusable, so the focus model never reaches the breakdown. The screen watcher
	/// edge-polls this on the results states and announces it. The Scoreboard instance is
	/// found in the live results-menu hierarchy; the rows are read in stored order (total
	/// first, then entries). Private fields are reached by reflection so a game-side
	...
	internal static class ScoreboardReader {
		...
		private static readonly FieldInfo ItemSubScoreField = AccessTools.Field(typeof(ScoreboardItem), "m_subScore");
		...
		public static void Read(out string header, out List<string[]> rows) {
			...
			foreach (ScoreboardItem item in items) {
				...
				rows.Add(new[] {
					...
					LabelText(ItemSubScoreField, item),
				});
			}
		}
