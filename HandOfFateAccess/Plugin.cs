...
namespace HandOfFateAccess {
	...
	public partial class Plugin : BaseUnityPlugin {
		...
		private void Initialize() {
			...
			// Non-speech audio backend (FMOD). Independent of speech (combat sonification needs
			// no screen reader), so it comes up regardless of the Tolk result. It is the seam
			// the combat and gambit layers register clips and play voices through. FMOD needs
			// its native dll preloaded from the plugins folder first, like Tolk and the SAPI
			// shim; if it fails to initialize the audio features log and stay silent.
			NativeLoader.Preload(pluginDir, "fmod.dll");
			AudioEngine.Initialize(new FmodAudioBackend());
			...
			// let those features degrade silently. Read from the backend's own mixer, not Unity's.
			if (AudioEngine.IsOutputMono)
